...
public interface    MovieRepository extends JpaRepository<Movie,Long> {
    ...
    @Query(nativeQuery = true,value = "SELECT distinct m.* FROM movie m left join mark ma on ma.movie_id = m.id left join tag t on t.id = ma.tag_id left join schedual s on s.movie_id=m.id left join cinema ci on s.cinema_id = ci.id" +
            " where " +
