...
public class MovieRecommender {
    ...
    @Resource(name = "elasticDataModel")
    private DataModel elasticDataModel;
    @Autowired
    ...
    public List<Long> userBasedRecommender(long userID,int size) throws TasteException {
        DataModel dataModel = mahoutConfig.getElasticDataModel();
        if(!mahoutConfig.preMap.containsKey(userID)){
            insertUser(userID);
            dataModel = mahoutConfig.getElasticDataModel();
        }
    ...
    public List<Long> itemBasedRecommender(long userID,int size) throws TasteException {
        DataModel dataModel = elasticDataModel;
        if(!mahoutConfig.preMap.containsKey(userID)){
            insertUser(userID);
            dataModel = mahoutConfig.getElasticDataModel();
        }
    ...
    private void insertUser(long userID){
        mahoutConfig.insertUser(userID);
    }
